package az.iba.ms.ufxinfo.handlers;

import static org.assertj.core.api.Assertions.assertThat;
import static org.mockito.Mockito.when;

import az.iba.ms.ufxinfo.dtos.ErrorResponseDto;
import az.iba.ms.ufxinfo.exceptions.FailedToGetSuccessfulResponseException;
...
public class CustomExceptionHandlerTest {
    ...
    @Test
    public void shouldReturn500_WhenExceptionIsThrown() {
    ...
    @Test
    public void shouldReturn400_WhenInvalidCardNumberExceptionIsThrown() {

        ResponseEntity<ErrorResponseDto> response =
                customExceptionHandler.handleException(new InvalidCardNumberException("Invalid card number 121312"));

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(response.getBody().getError()).isEqualTo(HttpStatus.BAD_REQUEST.name());
        assertThat(response.getBody().getMessage()).isEqualTo("Invalid card number 121312");
        assertThat(response.getBody().getErrorDetail()).isNull();
        assertThat(response.getBody().getStatus()).isEqualTo(HttpStatus.BAD_REQUEST.value());
        assertThat(response.getBody().getTimestamp()).isNotNull();
    }
    ...
    @Test
    public void shouldReturn504_WhenFailedToGetSuccessfulResponseExceptionIsThrown() {
